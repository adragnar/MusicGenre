...
def concat_data_and_gen_labels(dir_path):
    ...
    np.save("processed_data/final_data/all_songs.npy", all_song_data)
    np.save("processed_data/final_data/all_labels.npy", labels)
...
def split_data(feature_file, label_file):
    ...
    train_data, val_data, train_labels, val_labels = \
        train_test_split(features, labels, test_size=0.50, random_state=seed)

    np.save("./processed_data/final_data/train_data.npy", train_data)
    np.save("./processed_data/final_data/train_labels.npy", train_labels)
    np.save("./processed_data/final_data/val_data.npy", val_data)
    np.save("./processed_data/final_data/val_labels.npy", val_labels)


if __name__ == "__main__":
    concat_data_and_gen_labels("processed_data/normalized_data")
    split_data("processed_data/final_data/all_songs.npy", "processed_data/final_data/all_labels.npy")
